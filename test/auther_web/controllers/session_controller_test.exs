defmodule AutherWeb.SessionControllerTest do
  ...
  describe "POST #login" do
    ...
    test "with unkown email/password renders login form and flashes error", %{conn: conn} do
      conn = post(conn, Routes.session_path(conn, :login), email: "test@person.de", password: "irrelevant")
    ...
    test "with valid email/password redirects and flashes info", %{conn: conn} do
      ...
      {:ok, %User{}} = Accounts.create_user(
        %{name: "test", email: "test@user.de", password: "testing", password_confirmation: "testing"}
      )

      conn = post(conn, Routes.session_path(conn, :login), email: "test@user.de", password: "testing")
